...
def ddqn_train(model_name, load_model=False, model_filename=None, optimizer_filename=None):
    print("DDQN -- Training")
    ...
    for episode in range(start_episode + 1, end_episode + 1):
        ...
        if episode % 5000 == 0:
            agent.save_model_weights('models/ddqn_' + model_name + '_' + str(episode) + '.h5')
            agent.save_optimizer_weights('models/ddqn_' + model_name + '_' + str(episode) + '_optimizer.npy')

    agent.save_model_weights('models/ddqn_' + model_name + '_' + str(end_episode) + '.h5')
    agent.save_optimizer_weights('models/ddqn_' + model_name + '_' + str(end_episode) + '_optimizer.npy')
